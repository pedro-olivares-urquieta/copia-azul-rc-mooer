...
ROOT=Path('/mnt/data/reanalysis_cafe_azul');OUT=ROOT/'v10_2_results';AUD=ROOT/'v10_2_audio';CODE=ROOT/'v10_2_code';sys.path.insert(0,str(CODE));import build_v10_2 as m
...
metrics=pd.read_csv(OUT/'METRICAS_POR_PAREJA_V10_2.csv');metrics=metrics[~metrics.model.isin(['V9','V10_1'])];metrics=pd.concat([metrics,old_clean('V9'),old_clean('V10_1')],ignore_index=True);metrics.to_csv(OUT/'METRICAS_POR_PAREJA_V10_2.csv',index=False)
...
# comparison curves
v9=pd.read_csv(ROOT/'v9_results/curva_v9_puntos.csv');v10=pd.read_csv(ROOT/'v10_1_results/curva_v10_1_densa.csv')
plt.figure(figsize=(12,6));plt.semilogx(curve.frequency_hz,curve.precise_central_db,label='V10.2 CENTRAL');plt.semilogx(curve.frequency_hz,curve.precise_robust_db,label='V10.2 ROBUST');plt.semilogx(v10.frequency_hz,v10.eq_precise_db,label='V10.1');plt.semilogx(v9.frequency_hz,v9.eq_v9_db,label='V9');plt.axhline(0,color='k',lw=.6);plt.grid(alpha=.25,which='both');plt.legend();plt.xlabel('Hz');plt.ylabel('dB');plt.title('Curvas V9, V10.1 y V10.2');plt.tight_layout();plt.savefig(OUT/'10_COMPARACION_V9_V10_1_V10_2.png',dpi=180);plt.close()
# gain plot
...
# repackage analysis and complete
for zname,roots in [('/mnt/data/CAFE_AZUL_V10_2_ANALISIS_CODIGO.zip',[OUT,CODE]),('/mnt/data/CAFE_AZUL_V10_2_COMPLETA.zip',[OUT,CODE,AUD])]:
 try:os.remove(zname)
 except:pass
